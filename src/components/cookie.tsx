...
async function GetKuka() {
    const kuka = await getCookie('politic')
    return kuka
}

const Cookie = async () => {
    const kukuruza = await GetKuka()
    const soglasen = <Link href={'/privacy'} className={'text-green-500'}>сошлашение</Link>
    if (kukuruza === 'hidden') {
        return null
    } else {
        return (
