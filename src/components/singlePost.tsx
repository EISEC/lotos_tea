...
const SinglePost = ({data}: any) => {
    ...
    let imge = data[0]['_embedded']["wp:featuredmedia"][0].source_url === undefined ? false : data[0]['_embedded']["wp:featuredmedia"][0].source_url
    return (
        <main>
            <header className={'container mx-auto relative h-64'}>
                <div className={'bg-black/50 w-full h-full px-6'}>
                    {imge ? <Image className={'-z-10 absolute'} src={imge} alt={title} fill={true} loading="lazy"
